  ...
  return (
    <AppLayout title={t('floorSettings.title')}>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        ...
        {virtualDialog?.mode === 'create' && (
          <VirtualMemberFormDialog
            onClose={() => setVirtualDialog(null)}
            onSubmit={async (name, color) => {
              await createVirtualMember(name, color)
              showToast(t('floorSettings.virtual.createdToast', { name }), 'success')
            }}
          />
        )}
        {virtualDialog?.mode === 'edit' && (
          <VirtualMemberFormDialog
            member={virtualDialog.member}
            onClose={() => setVirtualDialog(null)}
            onSubmit={async (name, color) => {
              await updateVirtualMember(virtualDialog.member.id, name, color)
              showToast(t('floorSettings.virtual.updatedToast'), 'success')
            }}
          />
        )}
        {virtualDialog?.mode === 'link' && (
          <LinkVirtualDialog
            virtual={virtualDialog.member}
            realCandidates={members.filter((m) => !m.isVirtual)}
            onClose={() => setVirtualDialog(null)}
            onLink={async (virtualId, realId) => {
              await linkVirtualMember(virtualId, realId)
              showToast(t('floorSettings.virtual.linkedToast', { name: virtualDialog.member.name }), 'success')
            }}
          />
        )}
        {virtualDialog?.mode === 'remove' && (
          <ConfirmVirtualDialog
            danger
            title={t('floorSettings.virtual.removeTitle', { name: virtualDialog.member.name })}
            body={t('floorSettings.virtual.removeConfirm', { name: virtualDialog.member.name })}
            confirmLabel={t('floorSettings.virtual.removeYes')}
            workingLabel={t('floorSettings.virtual.removing')}
            onClose={() => setVirtualDialog(null)}
            onConfirm={async () => {
              await removeVirtualMember(virtualDialog.member)
              showToast(t('floorSettings.virtual.removedToast', { name: virtualDialog.member.name }), 'default')
            }}
          />
        )}
        ...
        <Reveal as="section" delay={160} className="card p-5">
          <div className="flex flex-wrap items-center justify-between gap-x-3 gap-y-2 mb-3">
            <h2 className="font-display font-semibold">{t('floorSettings.roommatesTitle')}</h2>
            {isAdmin && (
              <button type="button" className="btn-secondary text-xs px-3 py-1.5 shrink-0" onClick={() => setVirtualDialog({ mode: 'create' })}>
                {t('floorSettings.virtual.add')}
              </button>
            )}
          </div>
          <ul className="flex flex-col gap-2">
