...
export default function TaskCard({ task, typeInfo, assignee, currentUserId, onToggle }) {
  ...
  const typeLabel = typeInfo ? t(`taskTypes.${typeInfo.key}`) : ''

  function handleComplete() {
    setCelebrate(true)
    setTimeout(() => setCelebrate(false), 500)
    showToast(t('taskCard.completedToast', { label: typeLabel, points: typeInfo?.points }), 'success')
    onToggle(task.id)
  }
          ...
          <Link to="/compras" className="flex items-center gap-3 hover:opacity-80" title={t('taskCard.goToShopping')}>
            ...
            <div>
              ...
              <p className="flex items-center gap-1 text-xs text-ink-900/50 dark:text-cream-100/50">
                <CoinIcon className="w-3.5 h-3.5" />{t('taskCard.rewards', { points: typeInfo?.points })}
              </p>
            </div>
          </Link>
        ) : (
